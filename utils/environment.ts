...
const getEnvironmentConfig = (): EnvironmentConfig => {
  const env =
    Constants.expoConfig?.extra?.ENV || process.env.EXPO_PUBLIC_ENV || "local";
  ...
  const config: Record<string, EnvironmentConfig> = {
    local: {
      ...
      API_BASE_URL:
        ...
        "https://allio-app-bxwta.ondigitalocean.app/api",
      APP_NAME: process.env.EXPO_PUBLIC_APP_NAME || "AllioExpo Local",
      FIREBASE_PROJECT_ID:
        process.env.EXPO_PUBLIC_FIREBASE_PROJECT_ID || "allio-cd2b5",
      GOOGLE_WEB_CLIENT_ID:
        process.env.EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID ||
        "299086233123-40u7rfe1tdb4q5m7341rtdqo5qabf7eu.apps.googleusercontent.com",
      FACEBOOK_APP_ID:
        process.env.EXPO_PUBLIC_FACEBOOK_APP_ID || "1859280698339066",
